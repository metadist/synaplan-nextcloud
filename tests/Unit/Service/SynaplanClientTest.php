	...
	public function testUploadFileCallsCorrectEndpoint(): void
	{
		...
		$this->httpClient->expects($this->once())
			...
			->with(
				...
				$this->callback(function (array $options): bool {
					return $options['headers']['X-API-Key'] === 'sk_test_key_123'
						&& str_contains($options['headers']['Content-Type'], 'multipart/form-data')
						&& str_contains($options['body'], 'test.txt')
						&& str_contains($options['body'], 'RESEARCH')
						&& str_contains($options['body'], 'Hello world');
				})
			)
			->willReturn($response);
